 ...
 * @param {string} path 
 */
function generateAggregatorRecipe(ctx,path){
    ...
    addToJsonObject(json,"type","ae2cs:crystal_aggregator_recipe_serializer")
    ...
    addToJsonObject(json,"energy_cost",16000)
    addToJsonObject(json,"id",output.substring(output.indexOf(":") + 1))
    JsonIO.write(path,json)
    player.tell(Text.of(JsonIO.toPrettyString(json)).green().clickCopy(JsonIO.toPrettyString(json)).hover("Aggregator Recipe(Click to Copy)"))
}